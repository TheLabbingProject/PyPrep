...
class SubjectsAtlas:
    ...
    def copy_labels_to_subject(self, out_dir: Path):
        labels_file = out_dir / "atlas_labels.txt"
        if not labels_file.exists():
            print(f"Copying {self.atlas_labels} to {labels_file}")
            shutil.copy(self.atlas_labels, labels_file)

    def run(self):
        for subj, out_dir in zip(self.subjects, self.out_dir):
            self.copy_labels_to_subject(out_dir)
